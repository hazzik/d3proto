...
namespace d3emu
{
    ...
    class Password
    {
        ...
        public Password()
        {
            // Init client seed
            byte[] data = {
	            
            };
            //buffers[BufferIds.Buf_0x7A7D7B79] = GetRandomBytes(128);
            buffers[BufferIds.Buf_0x7A7D7B79] = data;
        }

        private void SetAccountName()
        {
            buffers[BufferIds.Buf_0x59F8A2FC] = Encoding.ASCII.GetBytes(accountName);
        }
        ...
        private void Handle_0x03_packet(BinaryReader reader)
        {
            ...
            if (!CheckAccount(data2, seed, secondChallenge, accountNameBytes))
            {
        ...
        private bool CheckAccount(byte[] secondProof, byte[] seed, byte[] secondChallenge, byte[] accountNameBytes)
        {
            var NBytes = new byte[]
                {
            ...
            var proof = secondProof.ToPosBigInteger();
            var challenge = secondChallenge.ToPosBigInteger();

            var temp1 = BigInteger.Remainder(challenge, N);

            if (BigInteger.Compare(challenge, 0) <= 0 || BigInteger.Compare(temp1, N) >= 0)
                return false;

            var Ndiv2 = N / 2;

            var temp2 = BigInteger.Remainder(proof, Ndiv2);

            //var sp = GetSecondChallenge(secondProof);

            //if (temp2 != sp)
            //    Console.WriteLine("bad");

            var temp3 = BigInteger.ModPow(challenge, Ndiv2, N);

            if (BigInteger.Compare(temp3, 1) >= 1 || BigInteger.Compare(temp2, Ndiv2) >= 0)
                return false;

            var HMAC = new HMACSHA512(accountNameBytes).ComputeHash(seed).ToPosBigInteger();

            var g2 = BigInteger.ModPow(g, proof, N);
            ...
            //var sc = GetSecondProof(accountNameBytes, seed, secondProof);

            //if (sc != g2)
            //    Console.WriteLine("bad");

            var temp4 = BigInteger.Remainder(challenge, g2);

            //if (BigInteger.Compare(temp4, g2) >= 1)
            if (challenge != g2)
                return false;
        ...
        public BigInteger GetSecondChallenge(byte[] seed)
        {
            var NBytes = new byte[]
                {
                    0xA3, 0x48, 0xF5, 0xDC, 0xFB, 0x38, 0x2D, 0x0B, 0x88, 0xA3, 0xC9, 0x32, 0xE9, 0x31, 0x58, 0x15,
            ...
            var temp2 = BigInteger.Remainder(seed.ToPosBigInteger(), Ndiv2);

            return temp2;
        }

        public BigInteger GetSecondProof(byte[] accountNameBytes, byte[] seed, byte[] secondProof)
        {
            var NBytes = new byte[]
                {
                    0xA3, 0x48, 0xF5, 0xDC, 0xFB, 0x38, 0x2D, 0x0B, 0x88, 0xA3, 0xC9, 0x32, 0xE9, 0x31, 0x58, 0x15,
